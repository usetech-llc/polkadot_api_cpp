...
RUN cd polkadot_api_cpp && git fetch --all && git checkout sr25519code && git submodule init && git submodule update --remote
RUN cd polkadot_api_cpp && cmake . && make clean && make sign_message && make transfer
