...
private:
    virtual Json getMetadata() {
        ...
        string testValue;
        string line;
        ifstream myfile("test/metadata/exampleMetadataV7.txt");
        if (myfile.is_open()) {
...
int main(int argc, char *argv[]) {
    ...
    if (resp2->metadataV7 != nullptr)
        cout << "V7 first attr - " << resp2->metadataV7->module[0]->name << endl;
    ...
    // metadataV6 is not set here
    assert(resp2->metadataV6 == nullptr);

    // metadataV7 parsed successfully, in other case it would be nullptr
    assert(resp2->metadataV7 != nullptr);
